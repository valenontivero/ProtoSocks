...
STRESS_TEST_OUTPUT_FILE=$(OUTPUT_FOLDER)/stress_test
STRESS_TEST_SOURCE=src/shared/test/stress_test.c

all: client server
server: $(SERVER_OUTPUT_FILE)
client: $(CLIENT_OUTPUT_FILE)
stress_test: $(STRESS_TEST_OUTPUT_FILE)
...
$(STRESS_TEST_OUTPUT_FILE): $(STRESS_TEST_SOURCE)
	mkdir -p $(OUTPUT_FOLDER)
	$(COMPILER) $(COMPILER_FLAGS) $(LD_FLAGS) $(STRESS_TEST_SOURCE) -o $(STRESS_TEST_OUTPUT_FILE)
...
tidy-fix:
	clang-tidy $(CLANG_TIDY_FIX_FLAGS) $(SERVER_SOURCES) $(CLIENT_SOURCES) $(SHARED_SOURCES) -- $(COMPILER_FLAGS)

format:
	clang-format -i $(SERVER_SOURCES) $(CLIENT_SOURCES) $(SHARED_SOURCES)

.PHONY: all server client stress_test clean tidy
