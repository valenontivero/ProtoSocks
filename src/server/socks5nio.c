...
static void on_hello_method(struct hello_parser *p, const uint8_t method)
{
	...
	else if (SOCKS_HELLO_NOAUTHENTICATION_REQUIRED == method && *selected == SOCKS_HELLO_NO_ACCEPTABLE_METHODS)
	{
		*selected = method;
	}
}
...
static bool auth_validate(const struct auth_parser *p)
{
	return user_store_validate(p->username, p->username_len, p->password, p->password_len);
}
...
static void auth_close(const unsigned state, struct selector_key *key)
{
	(void) state;
	(void) key;
}
...
static void *resolver_thread(void *data)
{
	...
	args->state->origin_resolution_error = getaddrinfo(args->host, args->service, &hints, &result);
...
static unsigned request_start_resolution(struct selector_key *key, struct request_st *d)
{
	struct socks5 *s = ATTACHMENT(key);
...
static uint8_t request_reply_for_connect_error(const int error)
{
	switch (error)
	{
		case ENETUNREACH:
...
static int request_build_sockaddr(const struct request_parser *p, struct sockaddr_storage *storage,
...
static int request_connect_sockaddr(struct selector_key *key, const int family, const struct sockaddr *addr,
									const socklen_t addr_len, bool *in_progress, int *connect_error)
{
	struct socks5 *s = ATTACHMENT(key);
	int fd;
	int ret;
	...
	*in_progress = ret < 0;
	s->origin_fd = fd;
	s->references++;
	if (SELECTOR_SUCCESS != selector_register(key->s, fd, &socks5_handler, *in_progress ? OP_WRITE : OP_NOOP, s))
	{
		*connect_error = errno;
		s->references--;
		s->origin_fd = -1;
...
static int request_connect_ip(struct selector_key *key, const struct request_parser *p, bool *in_progress,
							  int *connect_error)
{
	...
	*connect_error = 0;
	...
	return request_connect_sockaddr(key, family, (struct sockaddr *) &addr, addr_len, in_progress, connect_error);
}

static int request_connect_next_resolution(struct selector_key *key, bool *in_progress, int *connect_error)
{
	struct socks5 *s = ATTACHMENT(key);
	...
	*connect_error = EHOSTUNREACH;
	while (s->origin_resolution_current != NULL)
	{
		...
		if (request_connect_sockaddr(key, addr->ai_family, addr->ai_addr, addr->ai_addrlen, in_progress,
									 connect_error) == 0)
		{
			return 0;
		}
...
static int request_marshall_origin_bind(struct socks5 *s, struct request_st *d)
{
...
static unsigned resolving(struct selector_key *key)
{
	struct socks5 *s = ATTACHMENT(key);
...
static bool copy_buffer_can_write(buffer *b)
{
