...
void access_log_add(const char *username, const char *destination, const uint16_t port, const char *status)
{
	...
	{
		strftime(entry->timestamp, sizeof(entry->timestamp), "%Y-%m-%d %H:%M:%S", &tm_now);
	}
	else
	{
