...
static bool same_bytes(const uint8_t *a, const size_t a_len, const uint8_t *b, const size_t b_len)
{
	return a_len == b_len && memcmp(a, b, a_len) == 0;
}

static struct user_entry *find_user(const uint8_t *username, const size_t username_len)
{
	for (size_t i = 0; i < USER_STORE_MAX_USERS; i++)
	{
		if (users[i].used && same_bytes(users[i].username, users[i].username_len, username, username_len))
		{
			return &users[i];
		}
	}
...
bool user_store_validate(const uint8_t *username, const size_t username_len, const uint8_t *password,
						 const size_t password_len)
{
	...
	return user != NULL && same_bytes(user->password, user->password_len, password, password_len);
}

bool user_store_add(const uint8_t *username, const size_t username_len, const uint8_t *password,
					const size_t password_len)
{
	if (!valid_field(username, username_len) || !valid_field(password, password_len))
	{
...
bool user_store_remove(const uint8_t *username, const size_t username_len)
{
...
bool user_store_get_username(const size_t index, const uint8_t **username, size_t *username_len)
{
