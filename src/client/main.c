...
static void print_sock_banner(void)
{
	puts("⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣀⡠⢤⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀");
	puts("⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⡴⠟⠃⠀⠀⠙⣄⠀⠀⠀⠀⠀⠀⠀⠀⠀");
	puts("⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣠⠋⠀⠀⠀⠀⠀⠀⠘⣆⠀⠀⠀⠀⠀⠀⠀⠀");
...
static void print_help(void)
{
	...
	puts("  metrics                 Show proxy counters: connections and transferred bytes.");
	puts("  access-log              Show recent SOCKS connection attempts.");
	puts("  add-user <user> <pass>  Add or update a SOCKS authentication user.");
	puts("  del-user <user>         Remove a SOCKS authentication user.");
	puts("  list-users              List configured SOCKS authentication users.");
	puts("  exit                    Close the monitor client.");
}
...
int main(int argc, char **argv)
{
	...
	while (1)
	{
		...
		else if (strcmp(input, "access-log") == 0)
		{
			const char *access_log_cmd = "ACCESS-LOG\n";

			if (send(sock, access_log_cmd, strlen(access_log_cmd), 0) < 0)
			{
				perror("Failed to send ACCESS-LOG command");
				break;
			}

			char access_log_resp[16384] = {0};
			if (read_response(sock, access_log_resp, sizeof(access_log_resp)) > 0)
			{
				printf("%s", access_log_resp);
			}
			else
			{
				printf("Failed to receive ACCESS-LOG response\n");
				break;
			}
		}
		else
		{
			printf("Command not recognized. Type 'help' for a list of valid commands.\n");
		}
	}
