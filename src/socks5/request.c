...
enum request_state request_consume(buffer *b, struct request_parser *p, bool *errored)
{
	while (buffer_can_read(b) && p->state != REQUEST_DONE && p->state != REQUEST_ERROR)
	{
		...
		switch (p->state)
		{
			...
			case REQUEST_CMD:
				p->cmd = c;
				if (c != SOCKS_REQUEST_CMD_CONNECT && c != SOCKS_REQUEST_CMD_BIND &&
...
uint8_t request_reply_for(const struct request_parser *p)
{
	...
	if (p->atyp != SOCKS_REQUEST_ATYP_IPV4 && p->atyp != SOCKS_REQUEST_ATYP_DOMAIN &&
...
int request_marshall_ipv4(buffer *b, const uint8_t reply, const uint8_t addr[4], const uint16_t port)
{
...
int request_marshall_ipv6(buffer *b, const uint8_t reply, const uint8_t addr[16], const uint16_t port)
{
